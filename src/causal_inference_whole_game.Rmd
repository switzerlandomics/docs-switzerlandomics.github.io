...
# Causal inference - mosquito nets and malaria {#sec-whole-game}
...
This doc was built with: `rmarkdown::render("causal_inference_whole_game.Rmd", output_file = "../pages/causal_inference_whole_game.md")`

{: .warning }

This text is largely copied directly from the following source while we build an example closer to our needs. Please see the original source as follows. 

This example is described in the textbook:
Causal Inference in R, by
Malcolm Barrett,
...
Data [simulated by Dr. Andrew Heiss](https://evalsp21.classes.andrewheiss.com/example/matching-ipw/#program-background):
...
This data includes a variable that measures the likelihood of contracting malaria, something we wouldn't likely have in real life.
This let's us know the actual effect size to understand the methods better.
The simulated data is in `net_data` from the {[causalworkshop](https://github.com/r-causal/causalworkshop)} package, which includes ten variables:
...
In figure \@ref(fig:fig-malaria-risk-density), the density of those who used nets is to the left of those who did not use nets.
...
In figure \@ref(fig:fig-net-data-dag), we're saying that we believe:
...
When we calculate a naive linear regression that only includes net usage and malaria risk, the effect we see is incorrect because the seven other confounding pathways in figure \@ref(fig:fig-net-data-confounding) distort it.
...
```{r fig-mirror-histogram-net-data-unweighted}
...
```{r fig-mirror-histogram-net-data-weighted}
...
In this example, the unweighted distributions are not awful---the shapes are somewhat similar here, and they overlap quite a bit---but the weighted distributions in figure \@ref(fig:fig-mirror-histogram-net-data-unweighted) are much more similar.
...
0.1 is just a rule of thumb, but if we follow it, the variables in figure \@ref(fig:fig-love-plot-net-data) are well-balanced after weighting (and unbalanced before weighting).
...
The weights in figure \@ref(fig:fig-ate-density-net-data) are skewed, but there are no outrageous values.
...
```{r fig-bootstrap-estimates-net-data}
...
Figure figure \@ref(fig:fig-bootstrap-estimates-net-data) gives a sense of the variation in `estimate`, but let's calculate 95% confidence intervals from the bootstrapped distribution using rsample's `int_t()` :
...
The unmeasured confounder in figure \@ref(fig:fig-net-data-true-dag) is available in the dataset `net_data_full` as `genetic_resistance`.
...
We can dive more deeply into propensity score techniques, explore other methods for estimating causal effects, and, most importantly, make sure that the assumptions we're making are reasonable, even if we'll never know for sure.
